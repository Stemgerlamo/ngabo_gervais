...
public class CombatMission extends Mission {
    ...
    public boolean assignTask(String taskDescription, Personnel assignedTo) {
        ...
        if (assignedPersonnel.size() < 3) {
            System.out.println("Error: Combat mission requires at least 3 personnel.");
        ...
        System.out.println("Combat task assigned: " + taskWithAssignee);
        return true;
    }
    ...
    public boolean allocateResources(Resource resource) {
        ...
        if ((resource.getResourceType().equalsIgnoreCase("weapon") ||
                resource.getResourceType().equalsIgnoreCase("ammunition")) &&
                resource.getQuantity() > 0) {
            allocatedResources.add(resource);
            resource.setQuantity(resource.getQuantity() - 1);
            System.out.println("Combat resource allocated: " + resource.getResourceName());
            return true;
        } else {
            System.out.println("Error: Combat mission requires weapons or ammunition.");
            System.out.println("Available weapons/ammo: " +
                    (resource.getResourceType().equalsIgnoreCase("weapon") ||
                            resource.getResourceType().equalsIgnoreCase("ammunition") ? resource.getQuantity() : 0));
            return false;
        }
    }
